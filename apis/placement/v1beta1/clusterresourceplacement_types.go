...
type ClusterResourcePlacement struct {
	...
	// +kubebuilder:validation:XValidation:rule="!(has(oldSelf.statusReportingScope) && oldSelf.statusReportingScope == 'NamespaceAccessible' && has(oldSelf.resourceSelectors) && has(self.resourceSelectors) && size(oldSelf.resourceSelectors.filter(x, x.kind == 'Namespace')) == 1 && size(self.resourceSelectors.filter(x, x.kind == 'Namespace')) == 1 && oldSelf.resourceSelectors.filter(x, x.kind == 'Namespace')[0].name != self.resourceSelectors.filter(x, x.kind == 'Namespace')[0].name)",message="when statusReportingScope is NamespaceAccessible, the namespace name in resourceSelectors cannot be changed"
